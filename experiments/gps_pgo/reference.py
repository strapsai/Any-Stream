...
def solve(
    ...
    def residual(z):
        ...
        out = []
        for term in terms:
            kind, k, a, b, sigma = term[:5]
            j = term[5] if len(term) > 5 else k + 1
            pred = s[k] * (a @ R[k].T) + t[k]
            target = b if kind == 'gps' else s[j] * (b @ R[j].T) + t[j]
            r = ((pred - target) / sigma).ravel()
            if vector_loss:
                r = r.reshape(-1, 3)
                q = np.sum(r * r, axis=1)
                r *= np.sqrt(2 / (np.sqrt(1 + q) + 1))[:, None]
                r = r.ravel()
            elif block_loss and (not gps_only_block or kind == 'gps'):
                q = np.dot(r, r)
                r *= np.sqrt(2 / (np.sqrt(1 + q) + 1))
            out.append(r)
        for k in range(n - 1):
            dr = Rotation.from_matrix(R[k].T @ R[k + 1]).as_rotvec() / rot_sigma
            ds = (z[k + 1, 6] - z[k, 6]) / scale_sigma
            out.append(np.r_[dr, ds, 0.0, 0.0])
        if gravity_sigma is not None:
