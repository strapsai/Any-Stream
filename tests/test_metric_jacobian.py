...
@pytest.mark.parametrize('mode', [
    'radial', 'quadratic', 'quadratic_compressed', 'anchor_normalized', 'gps_bias', 'gravity',
    'fixed_scale', 'single_chunk'
])
def test_every_jacobian_column_against_central_differences(monkeypatch, mode):
    graph = problem()
    kwargs = {}
    graph['loops'] = [dict(i=0, j=1, a=graph['seams'][0][0], b=graph['seams'][0][1])]
    if mode.startswith('quadratic'):
        kwargs.update(vector_loss=False, compress=mode.endswith('compressed'))
    ...
    if mode == 'single_chunk':
        ...
        graph['gps'] = graph['gps'][:6]
        graph['valid_gps'] = graph['valid_gps'][:6]
        graph['seams'] = []
        graph['loops'] = []

    def check(fun, x0, **options):
        analytic = options['jac']
        rng = np.random.default_rng(73)
        for magnitude in [0., 1e-7, .4]:
            z = x0 + rng.normal(size=x0.shape) * magnitude
            actual = analytic(z).toarray()
...
def test_analytic_and_finite_difference_solution_and_world_frame_agree(x_scale_mode):
    graph = problem()
    a, da = metric_surface.solve(graph,
                                 jacobian_mode='finite_difference',
                                 x_scale_mode=x_scale_mode,
                                 max_nfev=200)
    b, db = metric_surface.solve(graph,
                                 jacobian_mode='analytic',
                                 x_scale_mode=x_scale_mode,
                                 max_nfev=200)
    assert da['success'] and db['success']
    assert da['initial_cost'] == db['initial_cost']
    assert abs(da['cost'] - db['cost']) < 1e-7
    for p, q in zip(a, b):
        np.testing.assert_allclose(transform(graph['seams'][0][0], p),
                                   transform(graph['seams'][0][0], q),
                                   atol=2e-4)
    ...
    c, dc = metric_surface.solve(other,
                                 jacobian_mode='analytic',
                                 x_scale_mode=x_scale_mode,
                                 max_nfev=200)
    assert abs(db['cost'] - dc['cost']) < 1e-6
    for p, q in zip(b, c):
        np.testing.assert_allclose(transform(graph['seams'][0][0], p) @ Q.T + t,
                                   transform(graph['seams'][0][0], q),
                                   atol=2e-4)
