...
def skew(v):
    v=np.asarray(v);out=np.zeros((*v.shape[:-1],3,3),dtype=float)
...
def right_jacobian(v,inverse=False):
    v=np.asarray(v,float);theta2=np.sum(v*v,axis=-1);theta=np.sqrt(theta2)
    small=theta2<1e-8;safe=np.where(small,1.,theta)
    K=skew(v);I=np.broadcast_to(np.eye(3),K.shape)
    if inverse:
...
def make_surface_jacobian(*,A,B,I,J,pivots,sigma,gps_points,anchor_points,gps_only_points,
                          nres,rot_sigma,scale_sigma,vector_loss,anchor_delta_m,
                          fix_scale=False,global_scale_sigma=0.,gravity_vectors=None,
                          gravity_sigma=None,gps_bias_sigma=None):
    n=len(pivots);m=len(A);nbias=3 if gps_bias_sigma is not None else 0
    free=~gps_points
    ...
    cols.append(np.broadcast_to(J[free,None,None]*7+np.arange(7),(int(free.sum()),3,7)).ravel())
    if nbias:
    ...
    for shift in [0,1]:
        ...
        cols.append(np.broadcast_to((np.arange(n-1)+shift)[:,None,None]*7+np.arange(3),(n-1,3,3)).ravel())
    for shift in [0,1]:
        rows.append(offset+np.arange(n-1)*6+3);cols.append((np.arange(n-1)+shift)*7+6)
    offset+=(n-1)*6
    if gravity_sigma is not None:
        ...
        cols.append(np.broadcast_to(np.arange(n)[:,None,None]*7+np.arange(3),(n,3,3)).ravel())
        offset+=3*n
    if global_scale_sigma>0:
        rows.append(np.full(n,offset));cols.append(np.arange(n)*7+6);offset+=1
    if nbias:
        rows.append(offset+np.arange(3));cols.append(n*7+np.arange(3));offset+=3
    assert offset==nres
    row=np.concatenate(rows);col=np.concatenate(cols)
    skew_a=skew(A);skew_b=skew(B[free]);eye=np.eye(3)

    def jacobian(z):
        zz=np.asarray(z[:n*7]).reshape(n,7);phi=zz[:,:3]
        R=Rotation.from_rotvec(phi).as_matrix();Jr=right_jacobian(phi)
        s=np.ones(n) if fix_scale else np.exp(zz[:,6]);t=pivots+zz[:,3:6]
        pa=s[I,None]*np.einsum('nij,nj->ni',R[I],A)
        pb=s[J,None]*np.einsum('nij,nj->ni',R[J],B)
        target=pb+t[J];target[gps_points]=B[gps_points]
        if nbias:target[gps_only_points]+=z[n*7:]
        raw=(pa+t[I]-target)/sigma
        left=np.zeros((m,3,7));left[:,:,:3]=-s[I,None,None]*(R[I]@skew_a@Jr[I])
        left[:,:,3:6]=eye
        ...
        if vector_loss:
            ...
            if anchor_delta_m is not None:alpha[anchor_points]=(sigma[anchor_points]/anchor_delta_m)**2
            q=np.sum(alpha*raw*raw,axis=1);h=np.sqrt(1+q);w=np.sqrt(2/(h+1))
            dw=-w/(4*h*(h+1))
            M=w[:,None,None]*eye+raw[:,:,None]*(2*dw[:,None]*alpha*raw)[:,None,:]
            M=M/sigma[:,None,:]
        else:
        ...
        values.extend([(-inv@rel.transpose(0,2,1)@Jr[:-1]).ravel(),(inv@Jr[1:]).ravel(),
                       np.full(n-1,-1/scale_sigma),np.full(n-1,1/scale_sigma)])
        if gravity_sigma is not None:
